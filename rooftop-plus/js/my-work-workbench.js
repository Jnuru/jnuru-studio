(function () {
  ...
  let _focusMode = sessionStorage.getItem("rooftop-focus-mode") !== "false";
  ...
  function getBuiltToday() {
    try {
      const stored = JSON.parse(sessionStorage.getItem("rooftop-built-today") || "{}");
  ...
  function recordBuiltToday() {
    try {
      ...
      sessionStorage.setItem("rooftop-built-today", JSON.stringify({ date: today, count }));
    } catch {}
  }
  ...
  function dealerShortName(dealer) {
    ...
    return source?.shortName || String(dealer || "Dealer").replace(/Redwood /i, "");
  }
...
<div class="focus-hero">
  ...
  document.addEventListener("click", (e) => {
    ...
    if (quickAction) {
      e.stopPropagation();
      _focusMode = false;
      sessionStorage.setItem("rooftop-focus-mode", "false");
    ...
    if (wbRow && !e.target.closest("[data-status]") && !e.target.closest("[data-aeo-status]") && !e.target.closest("a")) {
      ...
      _focusMode = true;
      sessionStorage.setItem("rooftop-focus-mode", "true");
      if (typeof renderMyWork === "function") renderMyWork(state.tasks);
      return;
    }
    ...
    if (e.target.closest("[data-focus-enter]")) { _focusMode = true; sessionStorage.setItem("rooftop-focus-mode", "true"); if (typeof renderMyWork === "function") renderMyWork(state.tasks); return; }
    if (e.target.closest("[data-focus-exit]"))  { _focusMode = false; sessionStorage.setItem("rooftop-focus-mode", "false"); if (typeof renderMyWork === "function") renderMyWork(state.tasks); return; }
  ...
  installMyWorkStyles();
  window.__rooftopMyWorkWorkbenchLoaded = true;
})();
