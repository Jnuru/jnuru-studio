...
function filteredTasks() {
  ...
  return state.tasks.filter((task) => {
    ...
    if (owner !== "all") {
      ...
      const isDevonAeoTask = owner === "Devon Price" &&
        !["done", "not_needed"].includes(task.aeoStatus) &&
        !["live", "ignored", "snoozed"].includes(task.pageStatus);
      if (taskOwner !== owner && !isDevonAeoTask) return false;
    }
...
const MY_WORK_GROUP_COLLAPSE_KEY = "rooftop-my-work-collapsed-groups";
...
function workspaceTitle(view) {
  return { my_work: "My Work", team_board: "Team Pipeline", wins: "Wins" }[view] || "My Work";
}

function workspaceSubtitle(view, role) {
  return { my_work: `${role || "Builder"} workspace`, team_board: "All active model page work", wins: "Celebrating what the team shipped" }[view] || "Builder workspace";
}
...
const PIPELINE_GROUP_COLLAPSE_KEY = "rooftop-pipeline-group-collapsed";

const OWNER_AVATAR_COLORS = {
  "Avery Coleman": "#4D8DF6",
  "Maya Torres":   "#3DB67A",
  "Devon Price":  "#9B5CF6",
};
...
function pipelineOwnerForTask(task) {
  if (["needs_seo", "seo_in_progress"].includes(task.pageStatus)) {
    return task.details?.seoOwner || "Maya Torres";
  }
  if (["seo_done", "needs_build", "page_built", "needs_review"].includes(task.pageStatus)) {
    return task.details?.buildOwner || "Avery Coleman";
  }
...
function renderTable(tasks) {
  ...
  // Shows tasks Devon still needs to complete AEO for
  const aeoTier = {
    key: "needs_aeo",
    title: "Needs AEO",
    hint: "AEO layer not yet complete — Devon\'s lane",
    dot: "teal",
    defaultOpen: false,
  };
  ...
  const aeoGroupHtml = renderPipelineGroup(aeoTier, aeoTasks, "Devon Price");
...
function dealerShortName(dealer) { const source = state.sources?.find((item) => item.dealer === dealer); return source?.shortName || String(dealer || "Dealer").replace("Redwood ", ""); }
...
function buildDigest() { const tasks = state.tasks.filter((task) => task.year === 2027); return ["Rooftop+ Daily Digest", `For: ${state.session?.name || "Team"} / ${primaryRole(state.session)}`, "", `Needs SEO: ${tasks.filter((task) => task.pageStatus === "needs_seo").length}`, ...tasks.filter((task) => task.pageStatus === "needs_seo").slice(0, 12).map((task) => `- ${task.dealer}: ${taskTitle(task)}`), "", `Ready / In Build: ${tasks.filter((task) => ["seo_done", "needs_build", "page_built"].includes(task.pageStatus)).length}`, "", `Live / Checked: ${tasks.filter((task) => task.pageStatus === "live").length}`].join("\n"); }
