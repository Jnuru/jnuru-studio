const ROOFTOP_THEME_KEY = "rooftop-theme";
...
function installSystemThemeListener() {
  ...
  const handleSystemThemeChange = () => {
    if (normalizeThemeChoice(localStorage.getItem(ROOFTOP_THEME_KEY) || state.selectedTheme) === "system") {
      applyTheme("system");
    }
  };
...
function setThemePreference(theme) {
  ...
  localStorage.setItem(ROOFTOP_THEME_KEY, choice);
  applyTheme(choice);
  return choice;
}
